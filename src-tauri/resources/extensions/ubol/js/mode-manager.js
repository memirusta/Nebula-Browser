...
// Nebula's application shell shares the WebView2 profile with browser tabs,
// and browser-verification endpoints require an unmodified execution context.
// Never run extension network or cosmetic filtering on these hosts.
const nebulaNoFilteringHostnames = [
    'tauri.localhost',
    'nebula.localhost',
    'asset.localhost',
    'challenges.cloudflare.com',
];
const nebulaCompleteFilteringHostnames = [ 'youtube.com' ];

function enforceNebulaInternalExceptions(filteringModes) {
...
export async function readFilteringModeDetails(bypassCache = false) {
    ...
    enforceNebulaInternalExceptions(userModes);
...
async function writeFilteringModeDetails(afterDetails) {
    enforceNebulaInternalExceptions(afterDetails);
